...
- **聊天窗口输出**：按分类汇总 + 数量概览
- **本地文档**：`~/news-aggregator/每日新闻/YYYY-MM-DD.md`
- **日志**：`~/.news-aggregator/logs/`（抓取与错误日志）
...
默认保存位置：`~/news-aggregator/每日新闻/YYYY-MM-DD.md`
...
- **定时任务未执行**：检查 `cron` 是否生效，查看 `~/.news-aggregator/logs/cron.log`
